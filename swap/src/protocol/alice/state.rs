...
use crate::aptos::wallet::{TransferRequest, WatchRequest};
use crate::aptos::TransferProof;
...
impl fmt::Display for AliceState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AliceState::AptLockTransactionSent { .. } => write!(f, "apt lock transaction sent"),
            AliceState::AptLocked { .. } => write!(f, "apt is locked"),
            AliceState::AptLockTransferProofSent { .. } => {
                write!(f, "apt lock transfer proof sent")
            }
            ...
            AliceState::AptRefunded => write!(f, "apt is refunded"),
...
impl State0 {
        ...
        R: RngCore + CryptoRng,
    {
        ...
        let s_a = aptos::Scalar::random(rng);
        let (dleq_proof_s_a, (S_a_bitcoin, S_a_aptos)) = CROSS_CURVE_PROOF_SYSTEM.prove(&s_a, rng);
        ...
        let valid = CROSS_CURVE_PROOF_SYSTEM.verify(
            ...
            (
                ...
                msg.S_b_aptos
                    .point
                    .decompress()
                    .ok_or_else(|| anyhow!("S_b is not a aptos curve point"))?,
            ),
        );
...
impl State2 {
    pub fn next_message(&self) -> Message3 {
        ...
        // Alice encsigns the refund transaction(bitcoin) digest with Bob's aptos
...
impl State3 {
    ...
    pub fn lock_apt_transfer_request(&self) -> TransferRequest {
        let S_a = aptos::PublicKey::from_private_key(&aptos::PrivateKey { scalar: self.s_a });

        let public_spend_key = S_a + self.S_b_aptos;
    ...
    pub fn extract_aptos_private_key(
        &self,
        published_refund_tx: bitcoin::Transaction,
    ) -> Result<aptos::PrivateKey> {
        self.tx_refund().extract_aptos_private_key(
            published_refund_tx,
            self.s_a,
            self.a.clone(),
            self.S_b_bitcoin,
    ...
    pub async fn refund_apt(
        ...
        // Ensure that the APT to be refunded are spendable by awaiting 10 confirmations
        // on the lock transaction
        aptos_wallet
            .watch_for_transfer(self.lock_apt_watch_request(transfer_proof, 10))
            .await?;
