    ...
    LR: LatestRate,
{
    ...
    Ok(match state {
        ...
        AliceState::BtcLocked { state3 } => {
            match state3.expired_timelocks(bitcoin_wallet).await? {
                ExpiredTimelocks::None => {
                    ...
                    let transfer_proof = aptos_wallet
                        .transfer(state3.lock_apt_transfer_request())
                        .await?;

                    AliceState::AptLockTransactionSent {
        ...
        AliceState::AptLockTransactionSent {
            ...
            ExpiredTimelocks::None => {
                aptos_wallet
                    .watch_for_transfer(state3.lock_apt_watch_request(transfer_proof.clone(), 1))
                    .await
                    .with_context(|| {
                        format!(
                            "Failed to watch for transfer of APT in transaction {}",
                            transfer_proof.tx_hash()
                        )
                    })?;

                AliceState::AptLocked {
                    aptos_wallet_restore_blockheight,
                    transfer_proof,
                    state3,
                }
        ...
        AliceState::BtcCancelled {
            ...
            select! {
                seen_refund = tx_refund_status.wait_until_seen() => {
                    ...
                    let spend_key = state3.extract_aptos_private_key(published_refund_tx)?;
        ...
        AliceState::AptRefunded => AliceState::AptRefunded,
