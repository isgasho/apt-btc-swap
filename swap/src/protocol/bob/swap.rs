...
pub async fn run_until(
    ...
    while !is_target_state(&current_state) {
        current_state = next_state(
            ...
            swap.aptos_wallet.as_ref(),
            swap.aptos_receive_address,
        )
        .await?;
...
async fn next_state(
    ...
    Ok(match state {
        ...
        BobState::SwapSetupCompleted(state2) => {
            // Record the current aptos wallet block height so we don't have to scan from
            ...
            // transaction, goes offline, while offline Alice publishes Aptos.
            // If the Aptos transaction gets confirmed before Bob comes online again then
            ...
            let aptos_wallet_restore_blockheight = aptos_wallet.block_height().await?;
        ...
        // Bob has locked Btc
        // Watch for Alice to Lock APT or for cancel timelock to elapse
        BobState::BtcLocked {
            state3,
            aptos_wallet_restore_blockheight,
        } => {
            ...
            if let ExpiredTimelocks::None = state3.current_epoch(bitcoin_wallet).await? {
                ...
                tracing::info!("Waiting for Alice to lock Aptos");

                select! {
                    transfer_proof = transfer_proof_watcher => {
                        let transfer_proof = transfer_proof?;

                        tracing::info!(txid = %transfer_proof.tx_hash(), "Alice locked Aptos");
                    ...
                    result = cancel_timelock_expires => {
                        result?;
                        tracing::info!("Alice took too long to lock Aptos, cancelling the swap");
        ...
        BobState::AptLockProofReceived {
            ...
            if let ExpiredTimelocks::None = state.current_epoch(bitcoin_wallet).await? {
                let watch_request = state.lock_apt_watch_request(lock_transfer_proof);

                select! {
                    received_apt = aptos_wallet.watch_for_transfer(watch_request) => {
                        match received_apt {
                            Ok(()) => BobState::AptLocked(state.apt_locked(aptos_wallet_restore_blockheight)),
                            Err(aptos::InsufficientFunds { expected, actual }) => {
                                tracing::warn!(%expected, %actual, "Insufficient Aptos have been locked!");
        ...
        BobState::BtcRedeemed(state) => {
            let (spend_key, view_key) = state.apt_keys();

            let wallet_file_name = swap_id.to_string();
            if let Err(e) = aptos_wallet
                .create_from_and_load(
                    wallet_file_name.clone(),
                    spend_key,
                    view_key,
                    state.aptos_wallet_restore_blockheight,
                )
                .await
            {
                ...
                // might not be able to ever transfer the Aptos.
                tracing::warn!("Failed to generate aptos wallet from keys: {:#}", e);
                tracing::info!(%wallet_file_name,
                    "Falling back to trying to open the the wallet if it already exists",
                );
                aptos_wallet.open(wallet_file_name).await?;
            }

            // Ensure that the generated wallet is synced so we have a proper balance
            aptos_wallet.refresh().await?;
            // Sweep (transfer all funds) to the given address
            let tx_hashes = aptos_wallet.sweep_all(aptos_receive_address).await?;

            for tx_hash in tx_hashes {
                tracing::info!(%aptos_receive_address, txid=%tx_hash.0, "Successfully transferred APT to wallet");
            }
        ...
        BobState::AptRedeemed { tx_lock_id } => BobState::XmrRedeemed { tx_lock_id },
    })
}
